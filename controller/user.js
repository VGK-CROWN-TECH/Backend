...
const { success, error, missingParam, created, noRecord } = require("../public/response");
...
let users = {
  ...
  userList: async (payload) => {
    let reqFields = isEmpty(payload, ["keyWord"]);
    if (reqFields.length > 0) {
      return missingParam(reqFields);
    }

    const searchTerm = payload.keyWord;
    ...
    try {
      ...
      if(users.length !== 0){
        return success("Login successful", users);
      } else{
        return noRecord();
      }
